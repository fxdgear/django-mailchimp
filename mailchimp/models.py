...
class QueueManager(models.Manager):
    def queue(self, campaign_type, contents, list_id, template_id, subject,  # NOQA
...
class Queue(models.Model):
    ...
    def send(self):
        ...
        camp = c.create_campaign(
            self.campaign_type, c.get_list_by_id(self.list_id),
...
class CampaignManager(models.Manager):
    def create(self, campaign_id, segment_opts, content_type=None, object_id=None, extra_info=[]):
        con = get_connection()
        camp = con.get_campaign_by_id(campaign_id)
        extra_info = simplejson.dumps(extra_info)
        obj = self.model(
            content=camp.content, campaign_id=campaign_id,
